...
export default function AddConsultation() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await consultationService.createConsultation(cleanedData);
  ...
  const canSubmit = () => {
    return formData.name && formData.category && formData.summary && 
           formData.about && formData.price && formData.image && 
  ...
  return (
    <div className="min-h-screen p-10 max-w-[1200px] mx-auto">
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-500">Create a new consultation service</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
              ...
              <div className="flex items-center space-x-2 bg-gray-100 p-1 rounded-xl">
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${
                    primaryImageMethod === 'url' ? 'bg-white text-zennara-green shadow-sm' : 'text-gray-600'
                  }`}
                >
                  URL
                </button>
                <button
                  ...
                  className={`px-4 py-1.5 rounded-lg text-sm font-semibold transition-all ${
                    primaryImageMethod === 'file' ? 'bg-white text-zennara-green shadow-sm' : 'text-gray-600'
                  }`}
                >
                  Upload
                </button>
          ...
          <div className="pt-8 border-t-2 border-gray-100">
            <h3 className="text-lg font-bold text-gray-900 mb-4">Additional Media Gallery</h3>
            ...
            <div className="flex items-center space-x-2 mb-6 bg-gray-100 p-1 rounded-2xl w-fit">
              <button
                ...
                className={`px-6 py-2 rounded-xl font-semibold ${
                  uploadMethod === 'url' ? 'bg-white text-zennara-green shadow-sm' : 'text-gray-600'
                }`}
              >
                Add by URL
              </button>
              <button
                ...
                className={`px-6 py-2 rounded-xl font-semibold ${
                  uploadMethod === 'file' ? 'bg-white text-zennara-green shadow-sm' : 'text-gray-600'
                }`}
              >
                Upload File
              </button>
            ...
            {uploadMethod === 'url' && (
              <div className="space-y-4 mb-6 p-6 bg-gray-50 rounded-2xl">
                <div className="grid grid-cols-3 gap-4">
                  <div className="col-span-2">
                    ...
                    <input
                      ...
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl font-semibold focus:outline-none focus:ring-2 focus:ring-zennara-green"
                    />
                  </div>
                  <div>
                    ...
                    <select
                      ...
                      className="w-full px-4 py-3 border-2 border-gray-200 rounded-xl font-semibold"
                    >
                ...
                <button
                  ...
                  className="w-full px-6 py-3 bg-gradient-to-r from-zennara-green to-emerald-600 text-white rounded-xl font-bold hover:shadow-lg disabled:opacity-50"
                >
            ...
            {uploadMethod === 'file' && (
              <div className="mb-6 p-6 bg-gray-50 rounded-2xl">
                <label className="block">
                  <div className="border-2 border-dashed border-gray-300 rounded-xl p-8 text-center cursor-pointer hover:border-zennara-green">
                    <p className="text-lg font-semibold text-gray-700 mb-2">
                      {uploading ? 'Uploading...' : 'Click to upload'}
                    </p>
                    <p className="text-sm text-gray-500">Images or Videos up to 50MB</p>
                  </div>
                  <input
                    type="file"
                    multiple
              ...
              <div className="grid grid-cols-3 gap-4">
                {formData.media.map((item, index) => (
                  <div key={index} className="relative group cursor-pointer" onClick={() => handlePreviewMedia(item, index)}>
                    {item.type === 'image' ? (
                      <img src={item.url} alt={`Media ${index + 1}`} className="w-full h-48 object-cover rounded-xl" />
                    ) : (
          ...
          {formData.key_benefits.map((benefit, index) => (
            <div key={index} className="flex items-center space-x-3 mb-3">
              <input
                ...
                className="flex-1 px-4 py-3 border-2 border-gray-200 rounded-2xl font-semibold focus:outline-none focus:ring-2 focus:ring-zennara-green"
              />
              {formData.key_benefits.length > 1 && (
                <button type="button" onClick={() => removeArrayItem('key_benefits', index)} className="p-3 bg-red-50 text-red-600 rounded-xl">
                  <XIcon className="w-5 h-5" />
                </button>
              )}
            </div>
          ))}
          <button type="button" onClick={() => addArrayItem('key_benefits')} className="mt-3 px-6 py-3 bg-gray-50 text-gray-700 rounded-2xl font-semibold">
            + Add Benefit
          </button>
        </div>
          ...
          {formData.ideal_for.map((item, index) => (
            <div key={index} className="flex items-center space-x-3 mb-3">
              ...
              {formData.ideal_for.length > 1 && (
                <button type="button" onClick={() => removeArrayItem('ideal_for', index)} className="p-3 bg-red-50 text-red-600 rounded-xl">
                  <XIcon className="w-5 h-5" />
                </button>
              )}
            </div>
          ))}
          <button type="button" onClick={() => addArrayItem('ideal_for')} className="mt-3 px-6 py-3 bg-gray-50 text-gray-700 rounded-2xl font-semibold">
            + Add Item
          </button>
        </div>
          ...
          {formData.tags.map((tag, index) => (
            <div key={index} className="flex items-center space-x-3 mb-3">
              ...
              {formData.tags.length > 1 && (
                <button type="button" onClick={() => removeArrayItem('tags', index)} className="p-3 bg-red-50 text-red-600 rounded-xl">
                  <XIcon className="w-5 h-5" />
                </button>
              )}
            </div>
          ))}
          <button type="button" onClick={() => addArrayItem('tags')} className="mt-3 px-6 py-3 bg-gray-50 text-gray-700 rounded-2xl font-semibold">
            + Add Tag
          </button>
        </div>
            ...
            <div key={index} className="mb-6 p-4 bg-gray-50 rounded-2xl">
              <div className="flex items-center justify-between mb-3">
                ...
                {formData.faqs.length > 1 && (
                  <button type="button" onClick={() => removeFAQ(index)} className="p-2 bg-red-50 text-red-600 rounded-xl">
                    <XIcon className="w-4 h-4" />
                  </button>
                )}
              </div>
              <input
                ...
                className="w-full px-4 py-3 mb-3 border-2 border-gray-200 rounded-2xl font-semibold focus:outline-none focus:ring-2 focus:ring-zennara-green"
              />
              <textarea
                value={faq.a}
                ...
                placeholder="Answer"
                rows="3"
                className="w-full px-4 py-3 border-2 border-gray-200 rounded-2xl font-semibold focus:outline-none focus:ring-2 focus:ring-zennara-green"
              />
            </div>
          ))}
          <button type="button" onClick={addFAQ} className="mt-3 px-6 py-3 bg-gray-50 text-gray-700 rounded-2xl font-semibold">
            + Add FAQ
          </button>
        </div>
      ...
      {showPreviewModal && previewMedia && (
        <div 
          ...
          onClick={() => { setShowPreviewModal(false); setPreviewMedia(null); }}
        >
          <div 
          ...
          >
            ...
            <div className="p-6 max-h-[60vh] overflow-auto">
              {previewMedia.type === 'image' ? (
                <img src={previewMedia.url} alt="Preview" className="w-full h-auto rounded-2xl" />
              ) : (
                <video src={previewMedia.url} controls className="w-full h-auto rounded-2xl bg-black" autoPlay />
              )}
            ...
            <div className="flex items-center justify-between p-6 border-t border-gray-200 bg-gray-50">
              <a
                ...
                className="px-6 py-3 bg-white border-2 border-gray-200 text-gray-700 rounded-2xl font-semibold hover:bg-gray-50"
              >
                Open in New Tab →
              </a>
              <button
                ...
                className="px-6 py-3 bg-red-500 text-white rounded-2xl font-semibold hover:bg-red-600 flex items-center space-x-2"
              >
                <XIcon className="w-5 h-5" />
